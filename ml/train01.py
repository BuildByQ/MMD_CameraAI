...
def main():
    ...
    # 1. 実際にCSVから抽出された「全カラム名」を取得
    feature_cols, label_cols = get_dynamic_columns(config, df.columns.tolist())
    
    # 2. その「実際の数」をベースに3倍する
    actual_base_dim = len(feature_cols) 
    input_dim = actual_base_dim * 3  # 生(1) + mean(1) + std(1)
    
    print(f"実際の抽出カラム数: {actual_base_dim}")
    print(f"モデル入力総次元 (input_dim): {input_dim}")

    label_columns = config.get("labels_features", [])
    if not label_columns:
        raise ValueError("config['labels_features'] が空です。")

    # 1. 実際にCSVから抽出された「全カラム名」を正確に取得
    feature_cols, label_cols = get_dynamic_columns(config, df.columns.tolist())
    
    # 2. 特徴量の総次元（生 + mean + std）
    actual_base_dim = len(feature_cols)
    input_dim = actual_base_dim * 3 
    
    # 3. ラベルの総次元
    output_dim = len(label_cols)
    
    print(f"特徴量カラム数: {actual_base_dim} -> 入力次元: {input_dim}")
    print(f"ラベルカラム数: {output_dim}")
    model = build_model(config, input_dim, output_dim, num_songs)

    # 6. 損失関数・最適化
    # label_cols = config["labels_features"] ★ここが原因
